...
public interface UserDao extends JpaRepository<User, Long>{
    ...
    JOIN users u ON u.id = ranked.id
    LEFT JOIN answer a ON u.id = a.user_id
    WHERE a.fecha_respuesta >= DATE_SUB(CURDATE(), INTERVAL (WEEKDAY(CURDATE())) DAY)
      AND a.fecha_respuesta < DATE_ADD(DATE_SUB(CURDATE(), INTERVAL (WEEKDAY(CURDATE())) DAY), INTERVAL 7 DAY)
    GROUP BY u.id, u.username, ranked.position
    ORDER BY ranked.position ASC, u.id ASC
    """,
			countQuery = """
